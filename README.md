...
从 [Releases](../../releases/latest) 下载 `KeyClick-macOS-universal-v1.1.8.zip`，解压后把 `KeyClick.app` 拖入“应用程序”。首次启动在系统设置中打开：
...
如果某一枚浮标看起来没有反应，可点该行的 **“试点”**：KeyClick 会切回目标窗口，使用与真实键盘触发完全相同的坐标和单击注入方式执行一次点击，并在设置页显示最终点击坐标。这样可直接区分“按键没有捕获”和“浮标位置没有放在可点击控件上”。点击模式中，命中的浮标会短暂闪绿作为即时反馈。
